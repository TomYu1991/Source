...
<nav class="navbar navbar-inverse display-default opnav-navbar navbar-fixed-top" role="navigation">
	<div class="topnav-navbar">
		<div class="navbar-header text-center">
			...
			<a class="navbar-brand J_menuItem" href="${ctx}/home" data-index="0" >宝&nbsp;钢&nbsp;德&nbsp;盛&nbsp;称&nbsp;重&nbsp;自&nbsp;动&nbsp;化及安环系统</a>
		</div>
	</div>
...
<script>
    ...
    function openPage(){
        jp.get("${ctx}/checkAddr", function (data) {
            if(data.success){
                var url1 = "${ctx}/gateconsign/consign/";
                var title1 = "进门门岗管理";
                var isNew = false;
                var url2 = "${ctx}/gatepass/passcheck/";
                var title2 = "出门门岗管理";
                var url3 = '${ctx}/userpasscord/userPassRecord/';
                var title3 = '员工通行记录';
                //一号岗亭
                if(data.object==4){
                    openTab(url1,title1,isNew);
                    openTab(url2,title2,isNew);
                }
                //一五号岗亭身份证刷卡器
                if(data.object==5){
                    openTab(url3,title3,isNew);
                }
                //六号岗亭出
                if(data.object==6){
                    openTab(url2,title2,isNew);
