...
$(function () {
	...
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
		alert("您还未安装过插件，请安装视频插件！");
		return;
	}
	
	// 初始化插件参数及插入插件 // 宽度高度
	WebVideoCtrl.I_InitPlugin(670,530, {
        iWndowType: 2,
	...
	if (-1 == WebVideoCtrl.I_CheckPluginVersion()) {
		//alert("检测到新的插件版本，请点击“视频加载”进行安装升级！");
		//return;
	}
...
function showOPInfo(szInfo) {
	...
	//$("#td_warning").attr("innerHTML",szInfo);
}
...
// 登录
function clickLogin(szip,szport,username,password) {
	var szIP = szip,
		szPort = szport,
		szUsername = username,
		szPassword = password;
...
// 开始预览
function clickStartRealPlay(channelsid,index) {

	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(index),
		szIP = loginip,
		iStreamType = 1,//码流类型1-主码流，2-子码流，默认使用主码流预览
		iChannelID = parseInt(channelsid, 10),//播放通道号，默认通道1
		bZeroChannel = false,
		szInfo = "";
	if ("" == szIP) {
		return;
...
// 停止预览  yyd 传参测试
function myclickStopRealPlay(Index) {
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(Index),
		szInfo = "";

	if (oWndInfo != null) {
		var iRet = WebVideoCtrl.I_Stop(Index);
		if (0 == iRet) {
			szInfo = "停止预览成功！";
		} else {
			szInfo = "停止预览失败！";
