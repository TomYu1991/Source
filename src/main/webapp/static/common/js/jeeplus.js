...
(function() {
    ...
    jp = {
	   ...
	   confirm:function(msg, succFuc, cancelFuc){//msg:询问信息， succFuc：点‘是’调用的函数， errFuc:点‘否’调用的函数
		   top.layer.confirm(msg,
		     { area: ['400px', '250px'],fontSize:'50px',icon: 2, title:'系统提示', btn: ['确认','取消'] //按钮
		     }, function(index){
        ...
        prompt:function (title, href) {
            var index = top.layer.prompt({title: title,value: '请输入' , formType: 2,maxlength:25}, function(text){
    ...
    //        }
	 ...
	 //   },
	   ...
	   /**用户选择框**/
        ...
        /**客户选择框**/
        openCustomerSelectDialog:function(isMultiSelect,ctype,yesFuc){
          top.layer.open({
            type: 2,
            area: ['900px', '560px'],
            title:"选择客户",
            auto:true,
            maxmin: true, //开启最大化最小化按钮
            content: ctx+"/customer/customer/customerSelect?isMultiSelect="+isMultiSelect+"&ctype="+ctype,
            btn: ['确定', '关闭'],
            yes: function(index, layero){
              var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
              var names = layero.find("iframe")[0].contentWindow.getNameSelections();
        ...
        /**统一客户选择框**/
        openUnifiedCustomerSelectDialog:function(yesFuc){
            top.layer.open({
                type: 2,
                area: ['1200px', '600px'],
                title:"选择客户",
                auto:true,
                maxmin: true, //开启最大化最小化按钮
                content: ctx+"/customer/customer/unifiedCustomerSelect",
                btn: ['确定', '关闭'],
                yes: function(index, layero){
	  ...
	  //获取字典标签
		...
		*    beformSubmit：执行提交操作前执行的方法
        ...
        download: function(url,data, name) {
            var $a = $("<a></a>").attr("href", url +"?data="+encodeURI(data)).attr("download", name);
            $a[0].click();
        },
        /**
    ...
    //阻止表单按回车键提交后再次弹出窗口的问题
    function stopEnter(index,layero){
