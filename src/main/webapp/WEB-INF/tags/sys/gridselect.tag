...
<%@ attribute name="conditionId" type="java.lang.String" required="false" description="级联查询ID"%>
<%@ attribute name="info" type="java.lang.String" required="false" description="提示需要先选择的级联信息"%>
    ...
    $(document).ready(function(){
        $("#${id}Button, #${id}Name").click(function(){
            ...
            if("${conditionId}" !=null && "${conditionId}" !=""){
                ...
                condition = "?condition="+$("#${conditionId}Id").val();
            }
            top.layer.open({
                type: 2,
                area: ['65%', '800px'],
                ...
                auto:true,
                name:'friend',
                content: "${ctx}/tag/gridselect?url="+encodeURIComponent("${url}"+condition)+"&fieldLabels="+encodeURIComponent("${fieldLabels}")+"&fieldKeys="+encodeURIComponent("${fieldKeys}")+"&searchLabels="+encodeURIComponent("${searchLabels}")+"&searchKeys="+encodeURIComponent("${searchKeys}")+"&isMultiSelected=${isMultiSelected? true:false}",
                btn: ['确定', '关闭'],
                yes: function(index, layero){
                    ...
                    for(var i=0; i<items.length; i++){
                        var item = items[i];
                        ids.push(item${fn:substring(name, fns:lastIndexOf(name, '.'), fn:length(name))});
