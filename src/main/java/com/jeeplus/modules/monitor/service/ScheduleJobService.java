...
public class ScheduleJobService extends CrudService<ScheduleJobMapper, ScheduleJob> {
	 ...
	 */
	@Transactional(readOnly = true)
	public String getjobstatus(ScheduleJob scheduleJob){

		//scheduler.
		//this.getAllScheduleJob();
		List <ScheduleJob> jobs1 = this.getAllRuningScheduleJob();
		List <ScheduleJob> jobs2 = this.getTriggersInfo();
		String relString = "";
		for(int i = 0 ; i < jobs1.size(); i++ ){
			ScheduleJob job = jobs1.get(i);
			relString = relString + String.valueOf(i)+". ";
			relString = relString + job.getName()+" ";
			relString = relString + job.getGroup()+" ";
			relString = relString + job.getStatus()+" ";
			relString = relString + job.getDescription()+" ";
			relString = relString + job.getCronExpression()+" ";
			relString = relString + job.getIsInfo()+" \n";

		}


		for(int i = 0 ; i < jobs2.size(); i++ ){
			ScheduleJob job = jobs2.get(i);
			relString = relString + String.valueOf(i)+". ";
			relString = relString + job.getName()+" ";
			relString = relString + job.getGroup()+" ";
			relString = relString + job.getStatus()+" ";
